...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  h1 { text-align: center; font-size: 1.4rem; font-weight: 400; color: #888; letter-spacing: 4px; text-transform: uppercase; margin-bottom: 30px; }
...
function drawChart(canvasId, data, labels, color) {
  ...
  labels.forEach((l, i) => {
    if (i % step === 0 || i === labels.length - 1) ctx.fillText(l, px(i), H - 6);
  });
  ctx.beginPath();
  ...
  data.forEach((v, i) => {
    ...
    ctx.fillStyle = i === data.length - 1 ? '#fff' : color;
    ctx.fill();
  });
}
...
async function fetchAll() {
  try {
    ...
    document.getElementById('time').textContent = '연결 오류';
  }
}
fetchAll();
